...
- **Session cookie:** If `validateGuestSessionFromRequest` succeeds, the author stored in `wishes.author_name` is **`rsvp.name`** for that session (client `authorName` is ignored). Same source as `@features/gallery-upload` confirm.
